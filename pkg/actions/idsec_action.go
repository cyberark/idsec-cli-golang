...
import (
	...
	clicommon "github.com/cyberark/idsec-cli-golang/pkg/common"
...
func NewIdsecBaseAction() *IdsecBaseAction {
	return &IdsecBaseAction{
		logger: sdkcommon.GetLogger("IdsecBaseAction", sdkcommon.Unknown),
	}
}
...
func (a *IdsecBaseAction) CommonActionsExecution(cmd *cobra.Command, execArgs []string, printUpgrade bool) {
	...
	a.logger = sdkcommon.GetLogger("IdsecBaseAction", sdkcommon.Unknown)
	...
	if config.IsInteractive() && printUpgrade {
		...
		isLatest, latestVersion, err := clicommon.IsLatestVersion()
		if err == nil && !isLatest {
