...
func (a *IdsecConfigureAction) runInteractiveConfigureAction(cmd *cobra.Command, configureArgs []string) (*models.IdsecProfile, error) {
	...
	if len(auth.SupportedAuthenticatorsList) == 1 {
		...
		// Use radio/switch selection instead of checkbox to enforce single selection
		selectedAuthName, err := args.GetSwitchArg(
			cmd,
			"authenticator",
			"Which authenticator would you like to configure (select one)",
			func() []string {
			...
			func() string {
				// Default to existing authenticator if profile has one
				for _, a := range auth.SupportedAuthenticatorsList {
					if _, exists := profile.AuthProfiles[a.AuthenticatorName()]; exists {
						return a.AuthenticatorHumanReadableName()
					}
				}
				// Default to first authenticator if none exists
				if len(auth.SupportedAuthenticatorsList) > 0 {
					return auth.SupportedAuthenticatorsList[0].AuthenticatorHumanReadableName()
				}
				return ""
			}(),
			true,
		...
		for _, a := range auth.SupportedAuthenticatorsList {
			if a.AuthenticatorHumanReadableName() == selectedAuthName {
				workWithAuthenticators = []string{selectedAuthName}
				break
			}
		}
		if len(workWithAuthenticators) == 0 {
			return nil, fmt.Errorf("selected authenticator not found: %s", selectedAuthName)
		}
	}
	for _, authenticator := range auth.SupportedAuthenticatorsList {
...
func (a *IdsecConfigureAction) runConfigureAction(cmd *cobra.Command, configureArgs []string) {
	...
	if err := profile.Validate(); err != nil {
		a.logger.Error("Profile validation failed: %v", err)
		panic(err)
	}
