...
func TestBuildCLIContextTags_ServiceOperationAndResource(t *testing.T) {
	...
	execCmd := &cobra.Command{Use: "exec"}
	serviceCmd := &cobra.Command{Use: "pcloud"}
	resourceCmd := &cobra.Command{Use: "safes"}
	operationCmd := &cobra.Command{Use: "create"}

	root.AddCommand(execCmd)
	execCmd.AddCommand(serviceCmd)
	serviceCmd.AddCommand(resourceCmd)
	resourceCmd.AddCommand(operationCmd)

	got := buildCLIContextTags(operationCmd, execCmd)
	if got[featureadoption.TagKeyCLIService] != "pcloud" {
		t.Fatalf("expected cls=pcloud, got %q", got[featureadoption.TagKeyCLIService])
	}
	if got[featureadoption.TagKeyCLIOperation] != "create" {
		t.Fatalf("expected clo=create, got %q", got[featureadoption.TagKeyCLIOperation])
	}
	if got[featureadoption.TagKeyCLIResource] != "safes" {
		t.Fatalf("expected clr=safes, got %q", got[featureadoption.TagKeyCLIResource])
	}
	if _, ok := got[featureadoption.TagKeyCLIVersion]; !ok {
		t.Fatal("expected clv tag to exist")
	}
}

func TestBuildCLIContextTags_SubResourceAggregation(t *testing.T) {
	t.Parallel()

	root := &cobra.Command{Use: "idsec"}
	execCmd := &cobra.Command{Use: "exec"}
	serviceCmd := &cobra.Command{Use: "pcloud"}
	resourceCmd := &cobra.Command{Use: "accounts"}
	subResourceCmd := &cobra.Command{Use: "members"}
	operationCmd := &cobra.Command{Use: "add"}

	root.AddCommand(execCmd)
	execCmd.AddCommand(serviceCmd)
	serviceCmd.AddCommand(resourceCmd)
	resourceCmd.AddCommand(subResourceCmd)
	subResourceCmd.AddCommand(operationCmd)

	got := buildCLIContextTags(operationCmd, execCmd)
	if got[featureadoption.TagKeyCLIResource] != "accounts-members" {
		t.Fatalf("expected clr=accounts-members, got %q", got[featureadoption.TagKeyCLIResource])
	}
}

func TestBuildCLIContextTags_WithoutExecInPath(t *testing.T) {
	t.Parallel()

	root := &cobra.Command{Use: "idsec"}
	serviceCmd := &cobra.Command{Use: "pcloud"}
	resourceCmd := &cobra.Command{Use: "safes"}
	operationCmd := &cobra.Command{Use: "list"}

	root.AddCommand(serviceCmd)
	serviceCmd.AddCommand(resourceCmd)
	resourceCmd.AddCommand(operationCmd)

	got := buildCLIContextTags(operationCmd, nil)
	if got[featureadoption.TagKeyCLIService] != "pcloud" {
		t.Fatalf("expected cls=pcloud, got %q", got[featureadoption.TagKeyCLIService])
	}
	if got[featureadoption.TagKeyCLIOperation] != "list" {
		t.Fatalf("expected clo=list, got %q", got[featureadoption.TagKeyCLIOperation])
	}
	if got[featureadoption.TagKeyCLIResource] != "safes" {
		t.Fatalf("expected clr=safes, got %q", got[featureadoption.TagKeyCLIResource])
	}
